...
import { BUNDLE_NAME } from '../shared/constants';

const thisBundle = (typeof nodecg !== 'undefined' && nodecg?.bundleName) ? nodecg.bundleName : BUNDLE_NAME;
...
export const integrationAuthStateReplicant = useReplicant<Record<string, boolean>>('integration_auth_state', thisBundle, { defaultValue: {} });
export const apiStatusReplicant = useReplicant<Record<string, 'connected' | 'disconnected' | 'error' | 'checking'>>('apiStatus', thisBundle, { defaultValue: { startgg: 'disconnected', challonge: 'disconnected' } });
