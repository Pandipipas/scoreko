import { createHandler } from './message-handler.js';
...
const OLD_KEY_FILE = path.join(process.cwd(), 'cfg', 'scoreko-encryption-key.txt');
const NEW_KEY_DIR = path.join(os.homedir(), '.scoreko');
const NEW_KEY_FILE = path.join(NEW_KEY_DIR, 'encryption-key.txt');
...
function getEncryptionKey(): Buffer {
  ...
  try {
    if (!fs.existsSync(NEW_KEY_DIR)) {
      fs.mkdirSync(NEW_KEY_DIR, { recursive: true, mode: 0o700 });
    }

    if (fs.existsSync(NEW_KEY_FILE)) {
      const hexKey = fs.readFileSync(NEW_KEY_FILE, 'utf-8').trim();
      encryptionKey = Buffer.from(hexKey, 'hex');
    } else if (fs.existsSync(OLD_KEY_FILE)) {
      const hexKey = fs.readFileSync(OLD_KEY_FILE, 'utf-8').trim();
      fs.writeFileSync(NEW_KEY_FILE, hexKey, { mode: 0o600 });
      fs.unlinkSync(OLD_KEY_FILE);
      encryptionKey = Buffer.from(hexKey, 'hex');
    } else {
      const newKey = crypto.randomBytes(32);
      fs.writeFileSync(NEW_KEY_FILE, newKey.toString('hex'), { mode: 0o600 });
...
const apiStatusReplicant = nodecg.Replicant<Record<string, ApiStatus>>('apiStatus', {
  defaultValue: { startgg: 'disconnected', challonge: 'disconnected' },
  persistent: false,
});
...
export const tokenStore = {
  setToken(integration: string, token: string): void {
    if (!token) {
      ...
      apiStatusReplicant.value[integration] = 'disconnected';
      return;
    }
    ...
    try {
      ...
      apiStatusReplicant.value[integration] = 'connected';
    } catch (err) {
      nodecg.log.error(`[token-store] Failed to encrypt token for ${integration}:`, err instanceof Error ? err.message : err);
    }
  },

  setApiStatus(integration: string, status: ApiStatus): void {
    apiStatusReplicant.value[integration] = status;
  },
...
nodecg.listenFor('syncAuthState', createHandler({
  handler: (_data: unknown, ack) => {
    sendAck(ack, null, authStateReplicant.value);
  }
}));
