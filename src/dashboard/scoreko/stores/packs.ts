...
import { installedPacksRevision } from '../../../browser_shared/fighting-characters-state';
import {
...
export const usePacksStore = defineStore('packs', () => {
  ...
  function fetchRegistry(): void {
    nodecg.sendMessage('fetchPackRegistry', undefined, (err: Error | null) => {
      if (err) {
        console.error('[usePacksStore] fetchPackRegistry failed:', err);
        notifyError('Failed to fetch pack registry: ' + err.message);
      }
    });
  }
  ...
  function downloadPack(packId: string): void {
    nodecg.sendMessage('downloadPack', packId, (err: Error | null) => {
      if (err) {
        console.error(`[usePacksStore] downloadPack "${packId}" failed:`, err);
        notifyError(t('packsDownloadFailed').replace('{error}', err.message));
      } else {
        notifySuccess(t('packsDownloadStarted').replace('{name}', packId));
      }
    });
  }
  ...
  function uninstallPack(packId: string): void {
    nodecg.sendMessage('uninstallPack', packId, (err: Error | null) => {
      if (err) {
        console.error(`[usePacksStore] uninstallPack "${packId}" failed:`, err);
        notifyError(t('packsUninstallFailed').replace('{error}', err.message));
      } else {
        notifySuccess(t('packsUninstallSuccess').replace('{name}', packId));
      }
    });
  }
  ...
  function updatePack(packId: string): void {
    nodecg.sendMessage('updatePack', packId, (err: Error | null) => {
      if (err) {
        console.error(`[usePacksStore] updatePack "${packId}" failed:`, err);
        notifyError(t('packsUpdateFailed').replace('{error}', err.message));
      } else {
        notifySuccess(t('packsUpdateStarted').replace('{name}', packId));
      }
    });
  }
  ...
  function cancelPackDownload(packId: string): void {
    nodecg.sendMessage('cancelPackDownload', packId, (err: Error | null) => {
      if (err) {
        console.error(`[usePacksStore] cancelPackDownload "${packId}" failed:`, err);
        notifyError(t('packsDownloadCancelFailed').replace('{error}', err.message));
      } else {
        notifySuccess(t('packsDownloadCancelled').replace('{name}', packId));
      }
    });
  }
