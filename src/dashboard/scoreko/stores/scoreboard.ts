...
const normalizeScoreboard = (input: unknown): Scoreboard => {
  ...
  return {
    ...
    leftScore: typeof candidate.leftScore === 'number' ? Math.min(99, Math.max(0, Math.floor(candidate.leftScore))) : 0,
    rightScore: typeof candidate.rightScore === 'number' ? Math.min(99, Math.max(0, Math.floor(candidate.rightScore))) : 0,
...
export const useScoreboardStore = defineStore('scoreboard', () => {
  ...
  const incrementLeftScore = () => {
    scoreboard.value = {
      ...scoreboard.value,
      leftScore: Math.min(99, scoreboard.value.leftScore + 1),
    };
  };
  ...
  const incrementRightScore = () => {
    scoreboard.value = {
      ...scoreboard.value,
      rightScore: Math.min(99, scoreboard.value.rightScore + 1),
    };
  };
  ...
  const leftScore = computed({
    ...
    set: (value: number) => {
      scoreboard.value = {
        ...scoreboard.value,
        leftScore: Math.min(99, Math.max(0, Math.floor(value))),
      };
    },
  });
  ...
  const rightScore = computed({
    ...
    set: (value: number) => {
      scoreboard.value = {
        ...scoreboard.value,
        rightScore: Math.min(99, Math.max(0, Math.floor(value))),
      };
    },
  });
